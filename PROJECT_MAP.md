...
materialGroups.js — CRUD групп материалов, копирование состава (POST /:id/copy), привязка/отвязка к 5 типам объектов (/bind)
...
29.07.2026	Сквозная интеграция Групп материалов для всех 5 типов объектов. 10-UNION калькуляция материалов. Выделение элементов с привязками жирным шрифтом (<strong>) во всех справочниках и тултипы предпросмотра. Верстка привязанных объектов и кнопок действий переведена в вертикальные столбцы.
03.08.2026	[ФИНАЛЬНАЯ СЕССИЯ] На вкладке «Группы материалов» добавлена кнопка «📋 Копировать» и бэкенд API POST /api/material-groups/:id/copy для клонирования состава группы в новую или существующую группу.
