...
## 20260723_145454 — "Компоненты шкафов" — 3 вкладки
### Страница components-cabinets.html (3 вкладки, отдельные файлы):

1. **templates-tab.html** (Компоненты шкафов)
   - Все столбцы: ID, Тип, Название, Производитель, Артикул, Цена, Вес, Q, LN, TM, Ссылка
   - Фильтры (3 шт.): Тип, Название, Производитель (модальные окна)
   - drag-and-drop + сортировка + статус-строка + БД

2. **types-tab.html** (Типы компонентов шкафов)
   - drag-and-drop + сортировка + статус-строка + БД (без фильтров)

3. **params-tab.html** (Параметры компонентов шкафов)
   - drag-and-drop + сортировка + статус-строка + БД (без фильтров)

### Дамп БД: bd_calc_20260723_145454.sql
### Файлы: calc_wardrobe_20260723_145454.tar.gz
