...
router.post('/', isAdmin, async (req, res) => {
  ...
  try {
    const result = await pool.query('INSERT INTO component_types (name) VALUES ($1) RETURNING *', [name]);
...
router.post('/import', isAdmin, async (req, res) => {
  try {
    ...
    for (const name of names) {
      try {
        await pool.query('INSERT INTO component_types (name) VALUES ($1) ON CONFLICT (name) DO NOTHING', [name]);
        added++;
      } catch (e) {}
    }
