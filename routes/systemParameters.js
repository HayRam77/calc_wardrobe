...
router.get('/', auth, async (req, res) => { try { const r = await pool.query('SELECT * FROM system_parameters ORDER BY name'); res.json(r.rows); } catch (e) { console.error(e); res.status(500).json({ message: 'Ошибка' }); } });
