...
router.post('/:id/blocks', auth, isAdmin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO project_blocks (cabinet_id, template_id, quantity, position)
       VALUES ($1, $2, $3, (SELECT COALESCE(MAX(position), -1) + 1 FROM project_blocks WHERE cabinet_id = $1))
...
// GET /api/cabinets/:id/systems — Получить системы шкафа
  ...
  try {
    ...
    if (cabinet_id !== undefined && cabinet_id != req.params.id) {
      ...
      const newLink = await client.query(
        ...
        [cabinet_id, oldData.system_id, name || oldData.name, description !== undefined ? description : oldData.description]
      );
      await client.query('COMMIT');
  ...
  try {
    ...
    const result = await pool.query(`
      SELECT
        ...
        '' as chain_type,
        COALESCE(pb.position, 9999) as pos
      FROM project_blocks pb
      JOIN block_templates bt ON pb.template_id = bt.id
      LEFT JOIN component_types ct ON bt.type_id = ct.id
      WHERE pb.cabinet_id = $1
      ...
      SELECT
        ...
        COALESCE(sbl.position, 9999) as pos
    ...
    let html = '<div class="table-container"><table class="data-table" id="cabinetBlocksTable"><thead><tr>' +
      '<th class="drag-handle" style="width:1%"></th>' +
      ...
      '<th data-sort="block_type" style="position:relative;">Тип комп. шкафа<span class="filter-icon" data-col="block_type" style="cursor:pointer;font-size:14px;margin-left:4px;" title="Фильтр по типу">⚙</span></th>' +
      '<th data-sort="block_name" style="position:relative;">Название<span class="filter-icon" data-col="block_name" style="cursor:pointer;font-size:14px;margin-left:4px;" title="Фильтр по названию">⚙</span></th>' +
