...
router.get('/', auth, async (req, res) => { try { const r = await pool.query('SELECT * FROM system_component_types ORDER BY name'); res.json(r.rows); } catch (err) { console.error(err); res.status(500).json({ message: 'Ошибка' }); } });
